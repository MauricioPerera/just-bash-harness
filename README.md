...
**Version:** 0.1.9 · **Status:** v0 contract complete + packaged + CI'd + polished + applicable_when filter + cross-session memory + memory search/stats/export + compaction + AES-256-GCM at rest + **retrieval bench with optional threshold gate**. 133/133 unit tests pass. End-to-end validated against real Gemma 4 26B on Cloudflare Workers AI subscribing the public `agent-skills-pack@v2.2.0`. Distributable as a `harness` binary via `npm run build`.
