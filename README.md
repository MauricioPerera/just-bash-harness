...
- **Encryption asymmetry: skills bank stays plaintext**: when `policy.encryption.enabled: true`, sessions and memory are AES-256-GCM at rest; the **skills bank is NOT encrypted** even though it houses `db approval_stats` (per-skill behavioral counters since v0.3.0). This is **deliberate**, not an oversight (decision recorded 2026-05-06; rationale in DESIGN §4.4). The threat-model summary: an attacker with read access to the skills bank dir typically also reaches the skill source code AND the env var holding `HARNESS_ENCRYPTION_KEY`, so encrypting `approval_stats` while leaving the source plaintext doesn't reduce attack surface in proportion to the cost. The asymmetry is acknowledged for evaluators with stricter threat models who should mitigate at the OS / filesystem layer (e.g. encrypted home directory) rather than expect harness-level coverage.
