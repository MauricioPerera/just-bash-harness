...
### Invariants touched (retro, per LESSONS.md doctrine #2)

The doctrine was authored in commit `713d2c8` (one commit before this release). It says: a feature that reuses existing primitives must include an "Invariants touched" section. v0.3.0 was the first opportunity to apply it. The defense "this is for future work, the v0.3.0 features were pre-tracked issues" is the comfortable exit that makes operational doctrines aspirational. So this section is added retroactively, as a precedent: the doctrine applies the moment it exists.

#### #5 — Cloudflare retry/backoff
- **AbortSignal propagation**: existing invariant. New retry sleep MUST honor the signal so Ctrl+C during backoff exits cleanly. Verified by `retry: aborted signal during backoff` test.
- **No retry mid-stream**: existing invariant on conversation state integrity. Once any TurnEvent is yielded, the request is in flight and a retry would corrupt state + re-charge tokens. Enforced by structuring the retry loop OUTSIDE the SSE reader.
- **`maxWallclockMs` budget**: retry sleeps count against the loop's wallclock; verified by the loop reading `Date.now() - startedAt` between iterations regardless of where time was spent.
- **No new persistence boundaries**: retries do not write anywhere new.

#### #3 — Approval-fatigue stats
- **Persistence boundaries**: NEW write path. `db approval_stats` collection in the **skills bank** (not session-scoped). Documented in TESTING.md + module header. NB: the skills bank does not use createBankBash encryption today, so the stats are written in plaintext even when policy.encryption.enabled=true (which only covers sessions/memory). This is consistent with how the skills bank itself works; if the skills bank ever goes encrypted, `approval_stats` will need to be in the rekey path (now wired in this same patch — see "skills" target on `harness rekey`).
- **Approval gate**: counter increment is a side effect of the audit callback, fail-closed (counter failure does NOT block the approval flow). Verified by the `audit` callback wrapping with try/catch + onError sink in `createApprovalStatsStore`.
- **Trust model**: counters do NOT influence approval decisions — the gate stays purely policy-driven. The suggester only emits YAML for the user to apply manually. No automatic policy mutation.

#### #2 Phase 1 — Secret redaction
- **Trust model**: the trust model in DESIGN §2 marks tool stdout as untrusted *for instruction following*. This feature extends that to *for data exfiltration into long-lived stores*. The mental model is unchanged in spirit (stdout was never trusted); the persistence behavior is what changed.
- **Persistence boundaries**: write paths into `db turns` (per-session) and memory (`db sources` via wiki) now flow through `scrubToolResult` first. Same number of write sites; new transform applied at each.
- **Provider feedback channel**: tool_result content blocks pushed into the provider's next turn now contain the scrubbed string. The LLM never sees the raw secret. This is the most important boundary — silently scrubbing only at persistence would leave the LLM context vulnerable.
- **Approval gate**: untouched. Scrubbing happens AFTER the approval decision, on the result of an already-allowed skill exec.
- **AbortSignal**: untouched. Scrubbing is synchronous + bounded.

#### #4 — `harness rekey`
- **Encryption invariant**: this is the feature. The invariant we're explicitly upholding is "data encrypted with key A becomes readable with key B after a successful rekey, and remains UNREADABLE with key A". Verified by the dry-run path: it decrypts every collection with the OLD key as a precondition.
- **Persistence boundaries**: new read+write path that touches every encrypted bank dir. The atomic rename window (mv → mv) is documented as not-strictly-atomic in module header.
- **No concurrency**: rekey explicitly refuses if any target dir was modified <60s ago — a best-effort detection of a concurrent harness process. Not a real lock; documented limitation.
- **Approval gate**: irrelevant — rekey is an offline maintenance command, no LLM, no skills.
- **Future deuda**: hardcoded collection lists (`SESSION_COLLECTIONS`, `MEMORY_COLLECTIONS`, `SKILLS_COLLECTIONS`). Doctrine #4 ("duplicate facts will desynchronize") applies here. Tracked: should switch to dynamic discovery via readdir on the bank dir when a future db API exposes collection introspection.

#### #1 — Compaction summary
- **Trust model**: NEW source of strings. The summary is generated by the LLM from dropped turns and prepended to subsequent system prompts. By the trust model, output of an LLM is untrusted — but here it's content the harness chose to add to the prompt. The escape: it's prepended to the SAME system prompt position as the recalled-memory block (also LLM-derived — turn-kind memories are user/assistant text). Same trust class, same handling. No new classification needed.
- **Persistence boundaries**: NEW write path. `compaction-summary` kind records in memory (`db sources` via wiki). One per compaction event. Recallable in future turns.
- **Provider input**: a NEW provider call per compaction event. With `availableTools: []` so the summary call cannot fire tool calls (closing what would be a recursive-loop hazard). Verified by `runCompactionSummary` test.
- **`maxWallclockMs` budget**: the summary call counts against the same budget — verified by the call going through the same provider in the same tick.
- **AbortSignal propagation**: forwarded to the provider call; aborts during summary cancel the whole turn.
- **Approval gate**: untouched. Summary text is data, not a tool call.
- **Strict-additive guarantee**: when `summarize.enabled: false`, the v0.1.7 behavior must be preserved byte-for-byte. Now guarded by `scratch/live-test-summarize-disabled.ts` smoke (added post-publish in the same docs commit as this section).
